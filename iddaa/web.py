...
def uygulama_olustur():
    ...
    _en_iyi_hepsi = oneri.en_iyi_hepsi
    ...
    _fikstur_oranlari = oneri.fikstur_oranlari
    ...
    _en_iyi_oran = oneri.en_iyi_oran
    ...
    def sistem_onerisi():
        ...
        tarih = str(govde.get("tarih", ""))
        try:
            ayarlar = oneri.sistem_ayarlari(govde)
            site_oranlari = oneri.site_oranlari_ayikla(govde.get("site_oranlari"))
        except (TypeError, ValueError):
            return jsonify({"hata": "Geçersiz parametre."}), 400
        try:
            ...
            return jsonify({"hata": f"Fikstür alınamadı: {hata}"}), 502
        # Hesap gövdesi oneri.py'de: otomatik defterleme işi aynı fonksiyonu çağırır.
        return jsonify(oneri.sistem_onerisi_hesapla(df, fik, tarih, _DURUM["elo"], ayarlar, site_oranlari))
    ...
    def surpriz_radar():
        ...
        try:
            ...
            return jsonify({"hata": f"Fikstür alınamadı: {hata}"}), 502
        # Hesap gövdesi oneri.py'de (Sürpriz Radarı seçimleri de defterlenir).
        return jsonify(oneri.surpriz_radari_hesapla(df, fik, tarih))
